...
普通字符串和 Groovy 的 GStrings(interpolated strings, 内插字符串)是有区别的,
对于后者, 在判断(evaluating)整个标识符时内插值会被插入到最终的字符串中:
...
NOTE: 单引号字符串是 +java.lang.String+, 不支持内插.
...
NOTE: 三单引号字符串是 +java.lang.String+, 不支持内插.
